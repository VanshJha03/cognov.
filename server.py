...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://cognov.github.io", "http://localhost:5500", "http://127.0.0.1:5500"],
...
def handle_purchase_return(user_id: str, data: PurchaseReturnAction):
    ...
    add_journal_entry(user_id, "purchase_return", "Cash", "Inventory", amount, f"Return {data.qty} {data.item}")
    refresh_balance_sheet(user_id)
...
def handle_sales_return(user_id: str, data: SalesReturnAction):
    ...
    update_inventory(user_id, data.item, Decimal(data.qty))
    add_journal_entry(user_id, "sales_return", "Sales Returns", "Cash", revenue_return, f"Return {data.qty} {data.item}")
